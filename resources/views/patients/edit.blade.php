...
<div class="py-6">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-3xl font-extrabold text-gray-900">Edit Patient: {{ $patient->name }}</h2>
            ...
            <div class="px-4 py-5 sm:p-6">
                <form method="POST" action="{{ route('patients.update', $patient->id) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $patient->name) }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('name')
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="age" id="age" value="{{ old('age', $patient->age) }}" min="0" max="150" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            @error('age')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $patient->phone) }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('phone')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="address" id="address" rows="3" required class="mt-1 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ old('address', $patient->address) }}</textarea>
                        @error('address')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
